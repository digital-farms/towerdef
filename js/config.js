...
window.TOWER_SHOTS_LEFT = 30;                // количество выстрелов до исчезновения башни
...
window.TOWER_RANGE_MARGIN = 20;              // запас (в пикселях) поверх рассчитанного минимума
...
// --- База (главная башня) ---
// Максимальный и стартовый HP базы. При запуске ран-а base.hp устанавливается в BASE_HP_START,
// а изначально при загрузке страницы — в BASE_HP_MAX (для экрана ожидания).
window.BASE_HP_MAX = 1000;
window.BASE_HP_START = 1000;

// --- Стоимость подарков (TikTok) ---
// Эти значения используются для отображения и клиентской логики. Фактическая обработка
// на сервере читает значения из live.config.json (см. сервер), но держим их и здесь для согласованности UI.
window.GIFT_PRICE_TOWER_L1 = 1;   // цена подарка для Gift-башни уровня 1
window.GIFT_PRICE_TANK = 5;       // цена подарка для Танка
window.GIFT_PRICE_TOWER_L2 = 10;  // минимальная цена подарка для Gift-башни уровня 2

// --- Танк (моб за подарок 5) ---
// Вкл/выкл спавна танка по событию newGiftTank
window.TANK_ENABLED = true;
// Живучесть и движение
window.TANK_HP = 150;                   // HP танка
window.TANK_SPEED = 0.2;               // скорость движения (px/кадр)
window.TANK_CRUSH_RADIUS = 20;         // радиус «давки» башен
// Режим движения: 'horizontal' — ехать по прямой по оси X на уровне спавна; 'straight_to_base' — прямая к базе
window.TANK_PATH_MODE = 'horizontal';
// Остановки и выстрелы
window.TANK_STOP_EVERY_PX = 100;       // каждые N пикселей пути танк останавливается
window.TANK_STOP_DURATION_FRAMES = 100; // длительность остановки в кадрах (~0.66с при 60fps)
// Урон по базе
window.TANK_SHELL_DAMAGE = 10;         // урон от снаряда по базе
window.TANK_CONTACT_DAMAGE = 50;       // контактный урон, если танк доехал до базы
// Снаряд
window.TANK_SHELL_SPEED = 3;           // скорость снаряда (px/кадр)
window.TANK_SHELL_RADIUS = 10;          // радиус отрисовки снаряда
// Для горизонтального режима: допуск по вертикали для засчитывания попадания в базу
window.TANK_SHELL_HIT_TOLERANCE_Y = 60; // px
// Визуал
window.TANK_SCALE = 1.0;               // масштаб спрайта танка
window.TANK_DRAW_OFFSET_Y = 0;         // вертикальный сдвиг при отрисовке
// Анимации
window.TANK_RUN_FRAME_DELAY = 12;      // задержка между кадрами бега (2 кадра)
window.TANK_SHOT_FRAME_DELAY = 10;     // задержка между кадрами выстрела (4 кадра)

// Позиция дула танка (точка вылета пули), в координатах исходного кадра спрайта (до масштабирования)
// Для режима run и shot используем одну точку; при необходимости сделаем раздельные параметры
window.TANK_MUZZLE_X = 150;            // пиксели от левого края кадра
window.TANK_MUZZLE_Y = 40;             // пиксели от верхнего края кадра
